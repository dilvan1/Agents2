...
public class DesignCmp extends SVector<Component> {
	...
	protected static void makeLibrary(Component cmpIn, List<Component> library, List translationLst) {

		// library: [ component<Component> ... ]

		//    Find the component translation and if it was mirrowed
		Component cmp = cmpIn.clone();
	...
	library.add(cmp);
		...
		//    Find if component already in the library
		int aux1;
		for (aux1 = 0; aux1 < library.size() && !cmpIn.name.equals(library.get(aux1).name); aux1++) ;
		if (aux1 < library.size()) {
			Component libCmp = library.get(aux1);
	...
	List<ElectricNode> electricNodes;
	 ...
	 */
	@Override
	public void add(int ind, Component obj) {
		if (obj.reference == -1)
			throw new RuntimeException("Component with ref -1");
		super.add(ind, obj);
	}
	...
	margin= min((Consultant.getMinWidth(via)/2 +
	  ...
	  (Consultant.getMinWidth(via)/2 +
	   ...
	   Consultant.getWireWidth(layerTo)/2));
		...
		auxWire.get(1).width = widthL1;
		auxWire.get(2).width = widthVia;
		auxWire.get(3).width = widthL2;
		...
		for (int aux1 = 0; aux1 < 2 && !flagOk; aux1++) {
			...
			for (float aux2 = (float) 0.25; aux2 <= 1 && !flagOk; aux2 = aux2 + (float) 0.25) {
				...
				if (flagOk) {
					auxWire = getConnectionFromSegmentWireToPoint(wire, ind, auxWire1.get(1));
	...
	public void connectInSameLayer(Layer layer) {
		...
		for (int aux1 = 0; aux1 < electricNodes.size(); aux1++) {
			...
			ElectricNode node = getElectricNodes().get(aux1);
				...
				for (; !flagConnect && aux3 < nets.size(); ) {
					...
					for (int aux4 = 0; aux4 < net1.size() && !flagConnect; aux4++) {
						...
						for (int ind1 = 1; ind1 < wire1.size() && !flagConnect; ind1++) {
							if (!wire1.get(ind1).layer().equals(layer))
								continue;
							ele1.set(wire1.get(ind1 - 1), wire1.get(ind1), wire1.get(ind1).width());
							seg.c1 = ele1.getCenter();
							path[1] = new NoCrash(wire1, ele1, wire1.get(ind1).layer());
							...
							for (int aux5 = 0; aux5 < net2.size() && !flagConnect; aux5++) {
								...
								for (int ind2 = 1; ind2 < wire2.size() && !flagConnect; ind2++) {
									if (!wire2.get(ind2).layer().equals(layer))
										continue;
									ele2.set(wire2.get(ind2 - 1), wire2.get(ind2), wire2.get(ind2).width());
									seg.c2 = ele2.getCenter();
									path[0] = new NoCrash(wire2, ele2, wire2.get(ind2).layer());
									...
									if ((ele1.isVertical() || !ele1.isHorizontalAligned(ele2)) && ele1.isVerticalAligned(ele2)) {
										...
										int min1 = Math.min(wire1.get(ind1 - 1).y, wire1.get(ind1).y);
										int min2 = Math.min(wire2.get(ind2 - 1).y, wire2.get(ind2).y);
										int max1 = Math.max(wire1.get(ind1 - 1).y, wire1.get(ind1).y);
										int max2 = Math.max(wire2.get(ind2 - 1).y, wire2.get(ind2).y);
										...
										//    Chose the two extremes for the wire
										int min1 = Math.min(wire1.get(ind1 - 1).x, wire1.get(ind1).x);
										int min2 = Math.min(wire2.get(ind2 - 1).x, wire2.get(ind2).x);
										int max1 = Math.max(wire1.get(ind1 - 1).x, wire1.get(ind1).x);
										int max2 = Math.max(wire2.get(ind2 - 1).x, wire2.get(ind2).x);
	...
	Wire getConnectionFromSegmentWireToPoint(Wire wire, int ind, Pt pt) {
		...
		Wire auxWire = new Wire();
		Linea line1 = new Linea(wire.get(ind - 1), wire.get(ind));

		//    If wire segment isn't a segment
		if (line1.c1.equals(line1.c2)) {
			Layer layer = wire.get(ind).layer();
			int width = Consultant.db.getWireWidth(layer);
			...
			if (auxT.getT() < 0) {
				...
				if (!line2.c1.equals(line2.c2))
					auxWire.add(line2.c2, width, layer);
				auxWire.add(pt, width, layer);
			} else if (!line2.c1.equals(line2.c2)) {
				Layer layer = wire.get(ind).layer();
				int width = Consultant.db.getWireWidth(layer);
	...
	public List getCrashesPointer(NoCrash[] nets, Linea vect, Layer layer, int width) {
		...
		for(Component cmp: this)
			for (Wire wireTo : cmp.getBody()) {
				...
				//    Join the two listing keeping the correct order of elements
	...
	public List<ElectricNode> getElectricNodes() {
	...
	public Rectangle getEnvelope() {
		...
		Rectangle rec = null;
		for(Component cmp: this) {
			Rectangle auxRec = cmp.getEnvelope();
			if (auxRec == null)
				continue;
			if (rec == null) {
				rec = auxRec;
	...
	public int getEstimatedCrashes(Wire wire) {
		...
		for(Component cmp: this)
			for (Wire wireTo : cmp.getBody())
				numCrash = numCrash + wireTo.getEstimatedCrashes(wire);
		return numCrash;
	}
	...
	public int getWireCost(Wire wire) {
		...
		for (int aux1 = 1, last = 0; aux1 < wire.size(); aux1++, last++) {
			rec.set(wire.get(last), wire.get(aux1), wire.get(aux1).width());
			cost = cost + rec.getArea() * Consultant.db.getLayerCost(wire.get(aux1).layer());
		}
		return cost;
	}
	...
	public synchronized void initRoutingNets() {
		for (ElectricNode node : electricNodes)
			node.initRoutingNets(this);
	}
	...
	public boolean isCrashing(NoCrash[] nets, Wire wire) {
		for (Component cmp: this)
			for (Wire wire1 : cmp.getBody())
				if (wire1.isCrashing(nets, wire))
					return true;
		return false;
	}
	...
	public Wire makeConnection(NoCrash[] path, Linea pointer, Wire directWire, List lstTouch) {
		/*-
		 *    The 2 worst possible cases when a connection is approached:
		 *
		...
		WirePt ele = directWire.get(1);
		WirePt eleTo = wire2.get(touch);
		int width = Consultant.db.getMinWidth(ele.layer());
		Segment seg = new Segment(directWire.get(0), directWire.get(1));
		Wire newWire = new Wire();
		newWire.add(directWire.get(0).clone());
		newWire.add(directWire.get(1).clone());
		...
		//IF_DEBUG_ON(
		Dsp.get2().addRect(new Rectangle(newWire.get(0), newWire.get(1), newWire.get(1).width()), java.awt.Color.green);
		...
		if (lstCrash.isEmpty()) {
			...
			if (auxWire != null) {
				...
				auxWire = getConnectionFromSegmentWireToPoint(wire2, touch, newWire.get(newWire.size() - 1));
				if (auxWire.isEmpty())
					return newWire;
				if (!isCrashing(path, auxWire)) {
					auxWire.invert();
		...
		if (numberOfTries * step < 1 && !isCrashing(lstCrash, path, newWire)) {
			newWire.get(1).set(seg.getPointOfT(1 - step));
			...
			for (int aux2 = 0; aux2 < node.routingNets.size(); aux2++) {
				...
				for (int aux3 = 0; aux3 < net.size() && !flagOk; aux3++) {
					...
					for (int aux4 = 1; !flagOk && aux4 < wire.size(); aux4++) {
						...
						seg.c1 = wire.get(aux4 - 1);
						seg.c2 = wire.get(aux4);
	...
	public void printEdif(Writer out) throws IOException {
		...
		List<Component> library = new SVector<Component>();
		List<List> translationLst = new SVector<List>();
		...
		for (Component cmp: this)
			if (cmp instanceof Pad) {
				cmp.printEdif(out);
				translationLst.add(Gen.newList(cmp.name, new Integer(cmp.reference)));
			} else if (cmp instanceof ElectricNode) {
				if (cmp.getBody().isEmpty()) {
					flagStandardNode = true;
					translationLst.add(Gen.newList("stdNode", new Integer(cmp.reference)));
				} else {
					//String auxName((*this)[aux1].name());
					//Strstream auxStr((*this)[aux1].name());
					String auxName = cmp.name + cmp.reference;
					cmp.printEdif(out);
					//translationLst.ap( new_LIST((*this)[aux1].name(), (*this)[aux1].reference()));
					translationLst.add(Gen.newList(auxName, new Integer(cmp.reference)));
					//(*this)[aux1].name()= auxName;
				}
			} else
				makeLibrary(cmp, library, translationLst);
		...
		for (ElectricNode node: electricNodes) {
			...
			for (CompReg compReg: node.comps)
				out.write("                  (qualify cmp" + compReg.reference + " " + compReg.termName + ") \n");
			out.write("               ) \n");
		}
	...
	public synchronized void refElectricNodes() {
		electricNodes = new SVector<ElectricNode>();
		for (Component cmp: this)
			if (cmp instanceof ElectricNode)
				electricNodes.add((ElectricNode) cmp);
	}

	public synchronized void refFets() {
		fets = new SVector<Fet>();
		for (Component cmp: this)
	 ...
	 */
	@Override
	public Component set(int ind, Component obj) {
		if (obj.reference == -1)
			throw new RuntimeException("Component with ref -1");
