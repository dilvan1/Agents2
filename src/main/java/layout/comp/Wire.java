...
public class Wire extends OwnedList {
    ...
    public void drawOut(Display out) {
        ...
        for (int auxLast = 0, aux1 = 1; aux1 < size(); auxLast++, aux1++) {
            rec.set(at(auxLast), at(aux1), at(aux1).width());
            out.addRect(rec, at(aux1).layer().toString());
        }
    }
    ...
    public WiringPoint getClosestWiringPoints(Wire wire) {
        ...
        for (int aux1 = 1; aux1 < size(); aux1++) {
            if (!Consultant.db.isWiringLayer(at(aux1).layer())) continue;
            seg1.c1.set(at(aux1 - 1));
            seg1.c2.set(at(aux1));
            boolean seg1C1EqualsC2 = seg1.c1.equals(seg1.c2);
            for (int aux2 = 1; aux2 < wire.size(); aux2++) {
                if (!Consultant.db.isWiringLayer(wire.at(aux2).layer())) continue;
                seg2.c1.set(wire.at(aux2 - 1));
                seg2.c2.set(wire.at(aux2));
                double auxDist;
                if (seg1C1EqualsC2) {
                ...
                //IF_DEBUG_ON(
                //if (auxDist<0) {
                //    cerr << "auxDist: " << auxDist << endl;
                //    DRAW("(clearCanvas)")
                //    DRAW(*this)
                //    DRAW(wire)
                //    CHECK_ERR(true, " getClosestWiringPoints: The two nets more than touch each other, dist < 0");
                //}
                //)
    ...
    public List getCrashes(NoCrash[] nets, Wire wire) {
        ...
        for (int aux1 = 1; aux1 < size(); aux1++) {
            WirePt elem = at(aux1);
            rec.set(at(aux1 - 1), elem, elem.width());
            for (int last2 = 0, aux2 = 1; aux2 < wire.size(); last2++, aux2++) {
                if (testCrash(nets, rec, elem.layer(), wire.at(last2), wire.at(aux2))) {
    ...
    public Rectangle getEnvelope() {
        ...
        Rectangle rec = new Rectangle(at(0), at(1), at(1).width());
        Rectangle auxRec = new Rectangle(0, 0, 0, 0);
        for (int auxLast = 1, aux1 = 2; aux1 < size(); auxLast++, aux1++) {
            auxRec.set(at(auxLast), at(aux1), at(aux1).width());
    ...
    public Rectangle getEnvelope(Layer layer) {
        ...
        for (int auxLast = 0, aux1 = 1; aux1 < size(); auxLast++, aux1++) {
            if (!at(aux1).layer().equals(layer)) continue;
            if (rec == null) {
                rec = new Rectangle(at(auxLast), at(aux1), at(aux1).width());
                continue;
            }
            auxRec.set(at(auxLast), at(aux1), at(aux1).width());
            if (rec.c1.x > auxRec.c1.x) rec.c1.x = auxRec.c1.x;
    ...
    public int getEstimatedCrashes(Wire wire) {
        ...
        for (int last1 = 0, aux1 = 1; aux1 < size(); last1++, aux1++) {
            seg1.c1.set(at(last1));
            seg1.c2.set(at(aux1));
            for (int last2 = 0, aux2 = 1; aux2 < wire.size(); last2++, aux2++) {
                seg2.c1.set(wire.at(last2));
                seg2.c2.set(wire.at(aux2));
    ...
    public List getWiringLayersTouchesPointer(List lstRec, Linea vect, int width) {
        ...
        for (int aux1 = 1; aux1 < size(); aux1++) {
            ...
            recGen.set(at(aux1 - 1), elem, elem.width());
    ...
    public void invert() {
        ...
        for (aux1 = 0, aux2 = 1; aux2 < size(); aux1++, aux2++) {
            at(aux1).width = at(aux2).width;
            at(aux1).layer = at(aux2).layer;
        }
        at(aux1).width = 0;
        at(aux1).layer = Layer.EMPTY;
    ...
    public boolean isConnected(Wire wire) {
        ...
        for (int auxLast1 = 0, aux1 = 1; aux1 < size(); auxLast1++, aux1++) {
            rec1.set(at(auxLast1), at(aux1), at(aux1).width());
            for (int auxLast2 = 0, aux2 = 1; aux2 < wire.size(); auxLast2++, aux2++) {
                if (at(aux1).layer().equals(wire.at(aux2).layer())) {
                    rec2.set(wire.at(auxLast2), wire.at(aux2), wire.at(aux2).width());
    ...
    public boolean isCrashing(NoCrash[] nets, Wire wire) {
        ...
        for (int last1 = 0, aux1 = 1; aux1 < size(); last1++, aux1++) {
            WirePt elem = at(aux1);
            rec.set(at(last1), elem, elem.width());
            for (int last2 = 0, aux2 = 1; aux2 < wire.size(); last2++, aux2++) {
                if (testCrash(nets, rec, elem.layer(), wire.at(last2), wire.at(aux2)))
                    return true;
            }
    ...
    public boolean isCrashing(NoCrash[] nets, Wire wire, int ind) {
        WirePt elem = at(ind);
        Rectangle rec = new Rectangle(at(ind - 1), elem, elem.width());
        for (int last2 = 0, aux2 = 1; aux2 < wire.size(); last2++, aux2++) {
            if (testCrash(nets, rec, elem.layer(), wire.at(last2), wire.at(aux2)))
                return true;
        }
    ...
    public boolean isJoinable(Wire wire) {
        ...
        if (size() == 0 || wire.size() == 0)
            return true;
        if (!getLastSegment().isJoinable(wire.at(0)))
            return false;
	...
	System.out.println("SHort circuit"+getLastSegment().layer()+ " + "+ wire.at(1).layer() + "   "+ wire.getLastSegment().layer());
	return false;
	}
	return true;
	*/
        ...
        for (int last1 = 0, aux1 = 1; aux1 < size(); last1++, aux1++) {
            ...
            for (int last2 = 0, aux2 = 1; aux2 < wire.size(); last2++, aux2++) {
                if (testCrash(nets, rec, elem.layer(), wire.at(last2), wire.at(aux2)) &&
                        !Consultant.db.isViaForLayer(elem.layer(), wire.at(aux2).layer())) {
                    return false;
                }
            }
        }
    ...
    public boolean isRewirable() {
        ...
        int aux1;
        for (aux1 = 1; aux1 < size() && (Consultant.db.isWiringLayer(at(aux1).layer()) || Consultant.db.isConnectionLayer(at(aux1).layer())); aux1++)
            ;
        if (aux1 < size())
            return false;

        //        Test if isn't all poly
        for (aux1 = 1; aux1 < size() && at(aux1).layer().equals(Layer.POLY); aux1++) ;
        return (aux1 < size());
    }
    ...
    public void printEdif(Writer out) throws IOException {
        ...
        for (int auxLast = 0, aux1 = 1; aux1 < size(); auxLast++, aux1++) {
            rec.set(at(auxLast), at(aux1), at(aux1).width());
            out.write("                  (figureGroup " + at(aux1).layer() +
    ...
    private boolean testKeepCrash(NoCrash[] net, Rectangle rec, Layer layer1, Layer layer2) {
        ...
        if (Consultant.db.unconnectableLayer(layer1) && Consultant.db.unconnectableLayer(layer2))
            return true;
        for (int aux1 = 0; aux1 < net.length; aux1++) {
            NoCrash aux = net[aux1];
