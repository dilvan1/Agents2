...
public class CompEdif extends LangEdif {
    ...
    protected List addComponent(String cellFunction, String cellName, List auxView, List auxOption) throws LangException {
        ...
        if (cellFunction.equals("NMOS") || cellFunction.equals("PMOS")) {
            ...
            if (cellFunction.equals("NMOS")) cellLst.add(new Fet(-1, -1, Fet.NMOS));
            else cellLst.add(new Fet(-1, -1, Fet.PMOS));
            ...
            if (cellFunction.equals("NPN")) cellLst.add(new Bipolar(-1, -1, Bipolar.NPN));
            else cellLst.add(new Bipolar(-1, -1, Bipolar.PNP));
        ...
        for (int aux1 = 0; aux1 < declare.size(); aux1++) {
            ...
            cmp.getTerms().at(aux1).name = new Symbol((String) declare.get(aux1));
            int aux2;
            ...
            if (aux2 < implem.size()) {
                ...
                cmp.getTerms().at(aux1).getBody().addAll(auxImplementation);
    ...
    protected void addView(String cellName, List list, List userData, Hashtable cell) throws LangException {
        ...
        String getView;
        if (cellFunction.equals("MAIN")) getView = "SYMBOLIC";
        else getView = "MASKLAYOUT";
    ...
    protected List makeContents(List list) throws LangException {
        ...
        for (int aux1 = 0; aux1 < list.size(); aux1++) {
            ...
            for (aux2 = 0; aux2 < inst.size(); aux2++) {
                ...
                for (aux3 = 0; aux3 < cmp.getTerms().size() && !((List) inst.get(aux2)).get(1).equals(cmp.getTerms().at(aux3).name.toString()); aux3++)
                    ;
                if (aux3 >= cmp.getTerms().size())
                    throw new LangException("edif CONTENTS: Unknown port in mustJoin");

                //    Put the node address in the term
                cmp.getTerms().at(aux3).electricNode = node.number;
				/*-
                ...
                if (node.comps == null) node.comps = new CompRegList();
