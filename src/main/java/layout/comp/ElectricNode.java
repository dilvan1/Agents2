...
public class ElectricNode extends Component {
    ...
    public Object clone() {
        ...
        for (int aux1 = 0; aux1 < comps.size(); aux1++) {
            node.comps.add(comps.at(aux1).clone());
        }
	...
	*/
                ...
                if (cmp != this) {
                    ...
                    if (ind == -1) {
                        for (int aux3 = 0; aux3 < newcmp.getTerms().size(); aux3++) {
                            ind = getInd(cmp.getTerms().at(aux3).getBody(), thisWire);
                            if (ind != -1) {
                                net.add(newcmp.getTerms().at(aux3).getBody().at(ind));
                                break;
                            }
                        }
                    } else
                        net.add(newcmp.getBody().at(ind));
                } else
                    net.add(node.getBody().at(ind));
            }
            node.routingNets.add(net);
        }
        return node;
    ...
    public Object getRoutingData() {
        ...
        for (int aux1 = 0; aux1 < comps.size(); aux1++) {
            rd.comps.add(comps.at(aux1).clone());
        }
    ...
    public void initRoutingNets(DesignCmp design) {
        while (!routingNets.isEmpty()) {
            for (int aux1 = 0; aux1 < getBody().size(); ) {
                if (routingNets.lst(0).contains(getBody().at(aux1)))
                    getBody().remove(aux1);
                else
                    aux1++;
            }
        ...
        for (int aux1 = 0; aux1 < comps.size(); aux1++) {
            Component cmp = design.getByReference(comps.at(aux1).reference);
            comps.at(aux1).routingNet = routingNets.size();
		/*-
		 * Change this:
		 ...
		 */
            ...
            auxLst.addAll(cmp.getTerms().at(comps.at(aux1).termName).getBody());
        }
    }
    ...
    public void joinRoutingNets(int n1, int n2) {
        ...
        for (int aux1 = 0; aux1 < comps.size(); aux1++) {
            if (comps.at(aux1).routingNet == n2)
                comps.at(aux1).routingNet = n1;
            if (comps.at(aux1).routingNet > n2)
                comps.at(aux1).routingNet--;
        }
    ...
    void setCompReference(Wire wire2, int ind) {
        ...
        int aux1;
        for (aux1 = 0; aux1 < comps.size() && comps.at(aux1).reference == cmp.reference && comps.at(aux1).termName.equals(term.name); aux1++)
            ;
        if (aux1 >= comps.size())
            throw new RuntimeException("Couldn't find comps reference");
        comps.at(aux1).routingNet = ind;
    }
    ...
    public void unwire() {
        ...
        for (int aux1 = 0; aux1 < getBody().size(); ) {
            if (getBody().at(aux1).isRewirable()) {
                net.remove(getBody().at(aux1));
                getBody().remove(aux1);
            } else
                aux1++;
        }
