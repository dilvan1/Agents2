...
class ConnectExpert implements Runnable {
    ...
    private List opList = Gen.newList();
    ...
    void operator_CHANGE_DIRECTION(Operator opRete, Wire wire, Pt pointer) {
        ...
        if (direction.x != 0) {
            ...
            if (dist1 * dist2 != 0 && dist1 / Math.abs(dist1) != dist2 / Math.abs(dist2)) distEdge = -1;
                ...
                if (Math.abs(dist1) < Math.abs(dist2)) distEdge = dist1;
                else distEdge = dist2;
            }
        } else {
        ...
        if (lstTouch.isEmpty()) {
            ...
            if (distTarget > minBlockDist) {
                ...
                if (distTarget < distCrash - 1) {
                    ...
                    if (distCrash - 1 > minBlockDist && crashCmp != -1) {
                        Operator op = new Operator("GOTO_XY", direction);
...
//VERIFICAR se esta realmente funcionando para a distancia minima
            ...
            for (touchInd = 0;
                 touchInd < 3 &&
                         ...
                         ((Crash) lstTouch.get(touchInd)).getWire().at(((Crash) lstTouch.get(touchInd)).getInd()).layer() != wirePt.layer();
                 touchInd++)
                ;
            ...
            seg.c2 = auxPointer.getPointOfT(distance + Consultant.db.getMinWidth(wire2.at(ind2).layer()));
...
//MUDAR
    ...
    void operator_GET_ROUND(Operator opRete, Wire wire) {
        ...
        Wire wire2 = cmp2.getBody().at(opRete.wire);
        int ind2 = opRete.element;

        int width = Consultant.db.getWireWidth(currentLayer);
        int margin = Consultant.db.getWiringMargin(currentLayer, wire2.at(ind2).layer());

        NoCrash[] paths = new NoCrash[1];
        paths[0] = new NoCrash(wire, ind);

        Rectangle object = new Rectangle();
        object.set(wire2.at(ind2 - 1), wire2.at(ind2), wire2.at(ind2).width());
...
//COUT("KILL <<<<<< : WireCOST: " << wireCost << " Savings: " << savings << " BestCost: " << bestCost << endl)
	 ...
	 */
        int dist = ((Crash) lstCrash.get(0)).getDistance() + 2 * directWire.at(1).width();
	 ...
	 */
        ...
        for (int aux1 = 0; aux1 < lstCrash.size() && dist >= ((Crash) lstCrash.get(aux1)).getDistance(); aux1++) {
            ...
            if (!wireCrash.isRewirable()) {
                ...
                Dsp.println("HAS NO WIRING LAYERS: " + wireCrash); //.at(1).layer());
                continue;
                //return false;
			/*
