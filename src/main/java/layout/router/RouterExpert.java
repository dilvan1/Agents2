...
public class RouterExpert implements Runnable {
	...
	static boolean makeWellSubstratContacts(DesignCmp design) {
		...
		Rectangle rec = null;
		for (Fet fet: design.getFets()) {
			if (! fet.tecn.equals(Fet.PMOS)) continue;
			for (int aux2 = 0; aux2 < fet.getBody().size(); aux2++) {
				Rectangle auxRec = fet.getEnvelope(Layer.PDIFF);
		...
		for (Component cmp: design.getElectricNodes())
		...
		nwell.get(1).setWidth(nwell.get(1).width() + 2 * Consultant.db.getMinOverlaping(Layer.NWELL, Layer.PDIFF));

		//  Find the VDD Pad
		for (aux1 = 0; aux1 < design.getPads().size() && !design.getPads().get(aux1).name.equals("VDD"); aux1++)
			;
		Pad pad = design.getPads().get(aux1);
		...
		lpad1:
			for (Wire wire : pad.getBody())
				for (int ind = 0; ind < wire.size(); ind++) {
					...
					Segment auxLine = new Segment(wire.get(ind - 1), wirePt);
		...
		for (aux1 = 0; aux1 < design.getElectricNodes().size() && !((Component) design.getElectricNodes().get(aux1)).name.equals("VDD"); aux1++)
			;
		ElectricNode node = design.getElectricNodes().get(aux1);
		...
		node.getBody().add(new Wire(
				...
				new Pt(contactVDD.getLastSegment().x, nwell.get(0).y),
		...
		//  Find VSS pad
		for (aux1 = 0; aux1 < design.getPads().size() && !design.getPads().get(aux1).name.equals("VSS"); aux1++)
			;
		Pad padVss = design.getPads().get(aux1);
		...
		//        Find the VSS node
		for (aux1 = 0; aux1 < design.getElectricNodes().size() && !design.getElectricNodes().get(aux1).name.equals("VSS"); aux1++)
			;
		ElectricNode nodeVss = design.getElectricNodes().get(aux1);
	...
	synchronized void addAgentInQueue(ConnectExpert agt, List unwireNodes) {
		...
		for (Object obj : unwireLst) {
			for (num1 = 0; num1 < nodeRoutingQueue.size() && !nodeRoutingQueue.get(num1).equals(obj); num1++)
				;
	...
	RouterData calculateRouterData(DesignCmp design, ElectricNode node, WiringPoint wp, int net1, int net2, Rectangle envlp1, Rectangle envlp2) {
		...
		WirePt wirePt1 = ((Wire) node.routingNets.lst(net1).get(wp.getWire1())).get(wp.getWirePt1());
		WirePt wirePt2 = wire2.get(wp.getWirePt2());
		 ...
		 *                               seg
		 */
		Linea seg = new Linea(wire2.get(wp.getWirePt2() - 1), wire2.get(wp.getWirePt2()));
		Pt targetPt;
	...
	boolean connectSubnets(DesignCmp design, ElectricNode node, int net1, int net2) {
		...
		for (int countFlips = 0; countFlips < 2; countFlips++) {
			...
			Wire wire = new Wire();
			wire.add(new WirePt(auxWire1.get(wp.getWirePt1() - 1), 0, Layer.EMPTY));
			wire.add(auxWire1.get(wp.getWirePt1()).clone());
			agentsQueue.clear();
			...
			while (!agentsQueue.isEmpty() && flagSearch) {
				...
				if (bestTotal == null) {
					if (loopCount > MAX_UNWIRE_LOOPS_WITHOUT_WIRE)
						flagSearch = false;
				} else if (loopCount > MAX_UNWIRE_LOOPS_WITH_WIRE)
					flagSearch = false;
		...
		node.getBody().get(0).join(bestTotal.getWire());
		...
		unwireNodes.invert();
		for (Object obj : unwireNodes) {
			ElectricNode nodeSave = (ElectricNode) design.getByReference(((Integer) obj).intValue());
			nodeSave.unwire();
	...
	void expert() {
		...
		List<ElectricNode> nodes = design.getElectricNodes();
	...
	synchronized void tryAsBestWire(int cost, Wire wire, int net) {
		...
		if (best != null && cost >= best.getCost())
			return;
		best = new BestWire(wire.clone(), net, cost);
