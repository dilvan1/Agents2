			...
			if (num2 != -1) {
				...
				if (line1.size() == 1) {
					if (line2.size() == 1) {
						...
						if (f1.getTerms().get(Fet.DRAIN).electricNode == f2.getTerms().get(Fet.DRAIN).electricNode)
							f1.swapDS();
						else if (f1.getTerms().get(Fet.SOURCE).electricNode == f2.getTerms().get(Fet.SOURCE).electricNode)
							f2.swapDS();
						else if (f1.getTerms().get(Fet.DRAIN).electricNode == f2.getTerms().get(Fet.SOURCE).electricNode) {
							f1.swapDS();
							f2.swapDS();
						}
					} else {
					...
					Fet f2 = (Fet) line2.get(0);
					if (f1.getTerms().get(Fet.SOURCE).electricNode == f2.getTerms().get(Fet.SOURCE).electricNode)
						f2.swapDS();
				}
	...
	List testConn(List fet1, List fet2, List lstTest) {
		...
		for (int aux1 = 0; aux1 < c1.size(); aux1++) {
			...
			for (int aux2 = 0; aux2 < c2.size(); aux2++) {
				...
				for (aux3 = 0;
						aux3 < lstTest.size() &&
						((Fet) c1.get(aux1)).getTerms().get((Symbol) lstTest.lst(aux3).get(0)).electricNode !=
						((Fet) c2.get(aux2)).getTerms().get((Symbol) lstTest.lst(aux3).get(1)).electricNode;
						aux3++)
					;
