	...
	EvalReport evalCircuit() {
		...
		for (int aux1 = 0; aux1 < design.getElectricNodes().size(); aux1++) {
			...
			while (nets.size() > 1) {
				...
				//                Finds new wire
				WirePt wirePt1 = ((Wire) nets.lst(0).get(0)).get(1);
				WirePt wirePt2 = ((Wire) nets.lst(1).get(0)).get(1);
				...
				nets.lst(0).add(((ElectricNode) design.getElectricNodes().get(aux1)).getBody().get(0));
			...
			for (int aux2 = 0; aux2 < order.size(); aux2++) {
				...
				for (int columnInd = 0; columnInd < line.size(); columnInd++) {
					...
					if ((!flagFlip && (line.get(columnInd) instanceof Component)) || (flagFlip && (line.get(line.size() - 1 - columnInd) instanceof Component))) {
						...
						Component cmp;
						if (!flagFlip)
							cmp = ((Component) line.get(columnInd)).clone();
						else
							cmp = ((Component) line.get(line.size() - 1 - columnInd)).clone();
